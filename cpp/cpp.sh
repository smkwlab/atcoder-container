#!/bin/bash
#
# C++ Compile Script for AtCoder Judge Environment
# Based on toml/config.toml compile section
#
...
if [ -f /etc/atcoder/install_dir.txt ]; then
    INSTALL_DIR="$(cat /etc/atcoder/install_dir.txt)"
else
    INSTALL_DIR="/opt/atcoder/gcc"
fi

# Check if full libraries are available
FULL_LIBS=false
if [ -d "${INSTALL_DIR}/lib" ] && [ -f "${INSTALL_DIR}/lib/libboost_system.a" ]; then
    FULL_LIBS=true
fi

# Basic build flags (always used)
BASIC_FLAGS=(
"-DATCODER"
"-DONLINE_JUDGE"
"-O2"
...
# Full build flags (used when libraries are available)
FULL_FLAGS=(
"-DNOMINMAX"
"-DOR_PROTO_DLL="
"-DPROTOBUF_USE_DLLS"
...
if [[ $AC_VARIANT == "gcc" ]]; then
    if [[ $FULL_LIBS == "true" ]]; then
        g++ ./Main.cpp -o a.out "${BASIC_FLAGS[@]}" "${FULL_FLAGS[@]}"
    else
        g++ ./Main.cpp -o a.out "${BASIC_FLAGS[@]}"
    fi
else
    if [[ $FULL_LIBS == "true" ]]; then
        clang++ std.pcm std.compat.pcm ./Main.cpp -o a.out "${BASIC_FLAGS[@]}" "${FULL_FLAGS[@]}"
    else
        clang++ std.pcm std.compat.pcm ./Main.cpp -o a.out "${BASIC_FLAGS[@]}"
    fi
fi
